...
double SensorModel::likelihood(const particle_t& sample, const lidar_t& scan, const OccupancyGrid& map)
{
    ...
    for (auto& ray : movingScan) {
        ...
        Point<double> pastEndpoint(ray.origin.x + (ray.range + (0.05 * std::sqrt(2)) ) * std::cos(ray.theta), ray.origin.y + (ray.range + (0.05 * std::sqrt(2))) * std::sin(ray.theta));
        Point<double> past = global_position_to_grid_position(pastEndpoint, map);
        Point<double> beforeEndpoint(ray.origin.x + (ray.range - (0.05 * std::sqrt(2)) ) * std::cos(ray.theta), ray.origin.y + (ray.range - (0.05 * std::sqrt(2))) * std::sin(ray.theta));
        ...
        if (map.logOdds(rayEnd.x, rayEnd.y) > 0.0) {
            ...
            if (pastOdds > 0.0) {
                scanScore += 0.2*pastOdds;
            }
            if (beforeOdds > 0.0) {
                scanScore += 0.2*beforeOdds;
            }
        }
